...
GLuint graphics::textures::load (const std::string& filename)
{
    ...
    if (image) {
        spdlog::info("Loading image '{}', width={} height={} components={}", filename, width, height, comp);
        ...
        spdlog::warn("Could not load texture: {}", filename);
    }
    return texture;
}
struct Image
...
GLuint graphics::textures::loadArray (bool filtering, const std::vector<std::string>& filenames)
{
    GLuint texture = 0;
    int max_width = 0;
    int max_height = 0;
    ...
    for (auto filename : filenames) {
        ...
        SPDLOG_DEBUG("Loaded image '{}', width={} height={} components={}", filename, w, h, c);
    ...
    SPDLOG_DEBUG("Loaded {} images into texture array ({}x{}x{})", images.size(), max_width, max_height, max_components);
